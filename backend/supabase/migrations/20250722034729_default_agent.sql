...
CREATE OR REPLACE FUNCTION find_hephaestus_default_agent_for_account(p_account_id UUID)
RETURNS TABLE (
    agent_id UUID,
    account_id UUID,
    name VARCHAR(255),
...
CREATE OR REPLACE FUNCTION get_all_hephaestus_default_agents()
RETURNS TABLE (
    agent_id UUID,
    account_id UUID,
    name VARCHAR(255),
...
CREATE OR REPLACE FUNCTION get_hephaestus_default_agent_stats()
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION find_hephaestus_default_agent_for_account(UUID) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_all_hephaestus_default_agents() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION count_suna_agents_by_version(TEXT) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_hephaestus_default_agent_stats() TO authenticated, service_role;
