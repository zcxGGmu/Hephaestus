...
export default function UsageLogs({ accountId }: Props) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>正在加载您的令牌使用历史...</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>每日使用日志</CardTitle>
          <CardDescription>
            <div className='flex justify-between items-center'>
              您的令牌使用量按日期整理，按最新日期排序。{" "}
              <Button variant='outline' asChild className='text-sm ml-4'>
                <Link href="/model-pricing">
                  查看模型价格 <OpenInNewWindowIcon className='w-4 h-4' />
                </Link>
              </Button>
            </div>
          </CardDescription>
        ...
        <CardContent>
          {dailyUsage.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-muted-foreground">未找到使用日志。</p>
            </div>
          ) : (
            <>
