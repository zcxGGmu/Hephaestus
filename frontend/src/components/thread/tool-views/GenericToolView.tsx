...
export function GenericToolView({
  ...
  const formatContent = (content: any, isAssistant: boolean = false) => {
    ...
    // Handle new format: assistant message with tool_calls
    if (isAssistant && parsedContent?.tool_calls && Array.isArray(parsedContent.tool_calls)) {
      ...
      if (toolCall?.function) {
        ...
        if (toolCall.function.arguments && Object.keys(toolCall.function.arguments).length > 0) {
          formatted.parameters = toolCall.function.arguments;
        }
    ...
    // Handle new format: tool result with name and content
    if (!isAssistant && parsedContent?.name && parsedContent?.content) {
      const formatted: any = {
        tool: parsedContent.name,
    ...
    if (typeof parsedContent === 'object') {
      ...
      if ('tool_name' in parsedContent || 'xml_tag_name' in parsedContent) {
        const formatted: any = {
          tool: parsedContent.tool_name || parsedContent.xml_tag_name || 'unknown',
        };

        if (parsedContent.parameters && Object.keys(parsedContent.parameters).length > 0) {
          formatted.parameters = parsedContent.parameters;
        }

        if (parsedContent.result) {
          formatted.result = parsedContent.result;
        }
      ...
      if ('content' in parsedContent && typeof parsedContent.content === 'object') {
  ...
  const formattedAssistantContent = React.useMemo(
    () => formatContent(assistantContent, true),
    [assistantContent],
  );
  const formattedToolContent = React.useMemo(
    () => formatContent(toolContent, false),
    [toolContent],
  );
